...
#@version: 
#@author: lipengtao
#@contact: lipengtao@bjgoodwill.com
#@software: PyCharm Community Edition
#@file: McCabeChecker.py
#@time: 21-11-1 下午3:16
"""
...
def mcCabeFile(filePath, options):
    code = _read(filePath)
...
def mcCabeDir(dirPath, options):
    if os.path.isdir(dirPath):
        for filePath in os.listdir(dirPath):
            mcCabeDir(filePath, options)
    else:
        mcCabeFile(dirPath, options)
...
def main(argv=None):
    ...
    if os.path.isdir(process_path):
        mcCabeDir(process_path, options)
    else:
        mcCabeFile(process_path, options)
